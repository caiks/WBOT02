...
So, while scanning single *level models* has brought us almost to qualitative features with moderate realtime compute, we will focus on using offline processing for *multi-level models* in the short term. Later on, when we come back to realtime vision processing, we may be able to use *slice* topologies as a substitute for scanning. Eventually an embodied wotbot with multi-modal and temporal *multi-level models* that can interact burstily with its environment and other agents will certainly need to decide its actions based on predictions.
