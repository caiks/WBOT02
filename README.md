...
In `actor001` the fixed *models* had many identical *events* in a *slice* from the opening introduction sequence, the closing credit sequence, and other pauses in the action. Some *slices* might even be identical to a single *event*. We would like to compare these *models* to those created in random or other modes. To see what is going on in a *model*  the `actor002` GUI displays the representations and *likelihoods* of the ancestors and siblings  and example *events* of the current centre's *slice*.

This ability to 'browse' the *model* enables us to make qualitative estimates of how generic the *model* is. Of course we do not want it to be too generic - the *model* would never become large  enough to be useful in the compute time available. So we need explore modes that focus the search on hot spots. These are rather like the fixed points, in that the *slices* will be more specialised, but also like the random points because the hotspots initially have a certain degree of arbitrariness about them.

 `actor002`  was developed to both create *models* and browse them, but the event loop architecture of a GUI means that the callbacks must remain short and so the active induce must be done in a separate thread. This causes the *model* to lag behind the *events*. Dealing with the lags and interruptions tends to make it difficult to reproduce experiments - especially as the *event* capture is by screen grab. In addition, occasional prompts to continue from the BBC website used for the Fireman Sam videos meant that a motion detector was required to pause when the action stops. The Windows O/S that was used for development was a destop O/S rather than a server O/S so there there were problems arising from the operating system's scheduling of long running processes. Ultimately wotbot will comprise a client-server architecture with the sensors and actuators on the client, e.g. a mobile phone, and the active system running on a server machine. 
 
 To that end  `actor003`  dispenses with the *model* browser parts of the GUI and can be run without a GUI at all. The *models* generated by `actor003` can be browsed in `actor002` because the active structure is the same. Also, `actor003` captures the *events* from a list of videos on the local filesystem, rather than requiring a user to start and stop the streaming videos. The capture, act, update and induce all are designed to run synchronously, so that the captures are regular. (Although, depending on the underlying video library for the O/S Qt sometimes runs the media player ayschronously in a separate thread.)  `actor003` can run on a server O/S and so the compute intensive scanning modes described below can be implemented on a much larger machine than is typical for a desktop. 

<!-- TODO 

To begin, let us compare actor001 and actor002
